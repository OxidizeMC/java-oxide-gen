...
///
/// ```ignore
/// use java_oxide::{Env, Arg};
/// use java_oxide::sys::{jboolean, JNI_TRUE};
/// use bindings::java::lang::Object;
/// use bindings::android::view::KeyEvent;
///
/// mod bindings; // Generated by `java-oxide-gen`
///
...
impl<'env> Env<'env> {
    ...
    ///
    /// When calling Java methods, `java-oxide` may need to resolve class names (as strings)
    ...
    ///
    /// `set_class_loader` allows you to set a `ClassLoader` instance that `java-oxide` will use to
    ...
    unsafe fn exception_to_string(self, exception: jobject) -> String {
        ...
        let message = ((**self.env).v1_2.CallObjectMethodA)(
            self.env,
            exception,
    ...
    pub unsafe fn require_class(self, class: &CStr) -> jclass {
        ...
        if !classloader.is_null() {
            ...
            let result: *mut _jobject = ((**self.env).v1_2.CallObjectMethodA)(
                self.env,
                classloader,
                cl_method,
                args.as_ptr(),
    ...
    pub unsafe fn require_method(
        ...
        let res =
            ((**self.env).v1_2.GetMethodID)(self.env, class, method.as_ptr(), descriptor.as_ptr());
        if res.is_null() {
            ((**self.env).v1_2.ExceptionClear)(self.env);
    ...
    pub unsafe fn require_field(self, class: jclass, field: &CStr, descriptor: &CStr) -> jfieldID {
        let res =
            ((**self.env).v1_2.GetFieldID)(self.env, class, field.as_ptr(), descriptor.as_ptr());
        if res.is_null() {
            ((**self.env).v1_2.ExceptionClear)(self.env);
            let class_name = self.get_class_name(class);
    ...
    pub unsafe fn get_object_field<R: ReferenceType>(
        self,
        this: jobject,
        field: jfieldID,
    ) -> Option<Local<'env, R>> {
    ...
    pub unsafe fn set_object_field<R: ReferenceType>(
        self,
        this: jobject,
        field: jfieldID,
        value: impl AsArg<R>,
    ...
    pub unsafe fn set_static_boolean_field(self, class: jclass, field: jfieldID, value: bool) {
        ((**self.env).v1_2.SetStaticBooleanField)(
            self.env,
            class,
            field,
            if value { JNI_TRUE } else { JNI_FALSE },
